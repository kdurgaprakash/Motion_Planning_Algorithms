...
class DStar:
    ...
    def process_state(self):
        ...
        popnode_neighbors = self.get_neighbors(pop_node)
        ...
        if k < pop_node.h:
            for neighbor in popnode_neighbors:
                if neighbor.h <= k and pop_node.h > neighbor.h + self.cost(pop_node,neighbor):
                    pop_node.h = neighbor.h + self.cost(pop_node,neighbor)
                    pop_node.parent = neighbor

        # If node k is the same as h (LOWER)
        if k == pop_node.h:
            for neighbor in popnode_neighbors:
                if neighbor.tag == "NEW" or neighbor.parent == pop_node and neighbor.h!=pop_node.h+self.cost(pop_node,neighbor) or neighbor.parent != pop_node and neighbor.h>pop_node.h+self.cost(pop_node,neighbor):
                    neighbor.parent = pop_node
                    self.insert(neighbor, pop_node.h + self.cost(neighbor,pop_node))
        # Else node k is smaller than h (RASIE)

        else:
            for neighbor in popnode_neighbors:
                if neighbor.tag == "NEW" or neighbor.parent == pop_node and neighbor.h != pop_node.h + self.cost(neighbor,pop_node):
                    neighbor.parent = pop_node
                    self.insert(neighbor, pop_node.h + self.cost(neighbor,pop_node))

                else:
                    if neighbor.parent != pop_node and neighbor.h > pop_node.h + self.cost(neighbor,pop_node):
                        self.insert(pop_node, pop_node.h)
                    else:
                        if neighbor.parent != pop_node and pop_node.h>neighbor.h+self.cost(neighbor,pop_node) and neighbor.tag =="CLOSED" and neighbor.h > k:
